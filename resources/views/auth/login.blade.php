...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('layuimini/css/layuimini.css?v=2.0.1') }}" media="all">
    ...
    <![endif]-->
    <style>
        .main-body {top:50%;left:50%;position:absolute;-webkit-transform:translate(-50%,-50%);-moz-transform:translate(-50%,-50%);-ms-transform:translate(-50%,-50%);-o-transform:translate(-50%,-50%);transform:translate(-50%,-50%);}
        ...
        .login-main .login-bottom .center .item .icon-1 {background:url("{{ asset('layuimini/images/icon-login.png') }}") no-repeat 1px 0;}
        .login-main .login-bottom .center .item .icon-2 {background:url("{{ asset('layuimini/images/icon-login.png') }}") no-repeat -54px 0;}
        .login-main .login-bottom .center .item .icon-3 {background:url("{{ asset('layuimini/images/icon-login.png') }}") no-repeat -106px 0;}
        ...
        body {background:url("{{ asset('layuimini/images/loginbg.png') }}") 0% 0% / cover no-repeat;position:static;font-size:12px;}
        input::-webkit-input-placeholder {color:#a6aebf;}
        input::-moz-placeholder {/* Mozilla Firefox 19+ */            color:#a6aebf;}
        input:-moz-placeholder {/* Mozilla Firefox 4 to 18 */            color:#a6aebf;}
        input:-ms-input-placeholder {/* Internet Explorer 10-11 */            color:#a6aebf;}
        input:-webkit-autofill {/* 取消Chrome记住密码的背景颜色 */            box-shadow:0 0 0 1000px white inset !important;-webkit-box-shadow:0 0 0 1000px white inset !important;}
        html {height:100%;}
        .login-main .login-bottom .tip {clear:both;height:16px;line-height:16px;width:288px;margin:0 auto;}
...
<div class="main-body">
    <div class="login-main">
        ...
        <form class="layui-form" style="text-align:center; width:100%;height:100%;margin:0px;margin-top: 50px;">
            <div class="layui-form-item layui-row" style="display:flex;justify-content:center;">
                <label class="layui-form-label" style="color:white;width:auto;"><i class="layui-icon layui-icon-website"></i> Choose Language</label>
                <div class="layui-input-inline">
                    <select lay-filter="languages">
                        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <option value="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}" {{ hash_equals(LaravelLocalization::getCurrentLocale(), $localeCode) ? 'selected' : '' }}>{{ $properties['native'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
